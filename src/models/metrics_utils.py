...
def create_summary_file(metrics: Dict, output_dir: Path):
    ...
    IMPORTANT: This function always uses metrics from optimal_aggregation['test_confusion_matrix']
    for slide/particle level metrics when available, to ensure consistency with the test results.
    """
    summary_file = output_dir / "metrics_summary.txt"
    latex_summary_file = output_dir / "metrics_latex.txt"
    
    # Extract test confusion matrix metrics if available for guaranteed consistency
    slide_metrics = {}
    particle_metrics = {}
    if 'optimal_aggregation' in metrics and isinstance(metrics['optimal_aggregation'], dict):
        agg = metrics['optimal_aggregation']
        if 'test_confusion_matrix' in agg and isinstance(agg['test_confusion_matrix'], dict):
            test_cm = agg['test_confusion_matrix']
            
            # For inflammation task
            if metrics.get('task') == 'inflammation':
                # Use test_confusion_matrix values for slide-level metrics
                slide_metrics = {
                    'accuracy': test_cm.get('accuracy', 0),
                    'sensitivity': test_cm.get('sensitivity', 0),
                    'specificity': test_cm.get('specificity', 0),
                    'f1': test_cm.get('f1', 0),
                    'auroc': test_cm.get('auc', 0)
                }
            # For tissue task
            elif metrics.get('task') == 'tissue':
                # Use test_confusion_matrix values for particle-level metrics
                particle_metrics = {
                    'accuracy': test_cm.get('accuracy', 0),
                    'sensitivity': test_cm.get('sensitivity', 0),
                    'specificity': test_cm.get('specificity', 0),
    ...
    with open(summary_file, 'w') as f:
        ...
        if metrics.get('task') == 'inflammation':
            ...
            # Use pre-extracted slide metrics from test_confusion_matrix if available
            if slide_metrics:
                # These are guaranteed to match the test confusion matrix
                for metric in metric_types:
                    if metric in slide_metrics:
                        value = slide_metrics[metric]
                        if isinstance(value, (int, float, np.floating)):
                            f.write(f"{metric.capitalize():10s}: {value:.2%}\n")
            else:
                # Fall back to metrics dict if test_confusion_matrix wasn't available
                for metric in metric_types:
                    key = f'slide_{metric}'
                    if key in metrics:
                        value = metrics[key]
            ...
            # Use pre-extracted particle metrics from test_confusion_matrix if available
            if particle_metrics:
                # These are guaranteed to match the test confusion matrix
                for metric in metric_types:
                    if metric in particle_metrics:
                        value = particle_metrics[metric]
                        if isinstance(value, (int, float, np.floating)):
                            f.write(f"{metric.capitalize():10s}: {value:.2%}\n")
            else:
                # Fall back to metrics dict if test_confusion_matrix wasn't available
        ...
        if 'optimal_thresholds' in metrics:
            for level, level_metrics in metrics['optimal_thresholds'].items():
                if isinstance(level_metrics, dict) and 'threshold' in level_metrics:
                    ...
                    f.write(f"{level.capitalize()} level: {level_metrics if isinstance(level_metrics, (int, float, np.floating)) else 0.5:.3f}\n")
                f.write("\n")
                
        # Add a specific section for validation AUCs
        f.write("\nVALIDATION AUC SCORES\n")
        f.write("-" * 20 + "\n")
        
        # Look for validation AUC values in metrics
        if "validation_tile_auroc" in metrics:
            f.write(f"Tile-Level AUC: {metrics['validation_tile_auroc']:.4f}\n")
        if "validation_particle_auroc" in metrics:
            f.write(f"Particle-Level AUC: {metrics['validation_particle_auroc']:.4f}\n")
        if "validation_slide_auroc" in metrics:
            f.write(f"Slide-Level AUC: {metrics['validation_slide_auroc']:.4f}\n")
        f.write("\n")
        ...
        if 'optimal_aggregation' in metrics:
            ...
            if isinstance(agg, dict):
                ...
                f.write(f"Aggregation info: {agg}\n")
            
            # === VALIDATION DATASET CONFUSION MATRIX ===
            if isinstance(agg, dict) and 'validation_confusion_matrix' in agg:
                validation_cm = agg['validation_confusion_matrix']
                if isinstance(validation_cm, dict):
                    f.write("\nVALIDATION DATASET CONFUSION MATRIX (Optimized on this data):\n")
                    f.write("-" * 55 + "\n")
                    f.write(f"  True Positives: {validation_cm.get('tp', 0)}\n")
                    f.write(f"  True Negatives: {validation_cm.get('tn', 0)}\n")
                    f.write(f"  False Positives: {validation_cm.get('fp', 0)}\n")
                    f.write(f"  False Negatives: {validation_cm.get('fn', 0)}\n")
                    f.write(f"  Total Samples: {validation_cm.get('total', 0)}\n")
                else:
                    f.write("\nVALIDATION DATASET CONFUSION MATRIX:\n")
                    f.write("-" * 35 + "\n")
                    f.write(f"  Data: {validation_cm}\n")
            
            # === TEST DATASET CONFUSION MATRIX ===
            if isinstance(agg, dict) and 'test_confusion_matrix' in agg:
                test_cm = agg['test_confusion_matrix']
                if isinstance(test_cm, dict):
                    f.write("\nTEST DATASET CONFUSION MATRIX (Using validation-optimized strategy):\n")
                    f.write("-" * 60 + "\n")
                    f.write(f"  True Positives: {test_cm.get('tp', 0)}\n")
                    f.write(f"  True Negatives: {test_cm.get('tn', 0)}\n")
                    f.write(f"  False Positives: {test_cm.get('fp', 0)}\n")
                    f.write(f"  False Negatives: {test_cm.get('fn', 0)}\n")
                    f.write(f"  Total Samples: {test_cm.get('total', 0)}\n")
                    f.write(f"  Accuracy: {test_cm.get('accuracy', 0):.2%}\n")
                    f.write(f"  F1 Score: {test_cm.get('f1', 0):.2%}\n")
                    f.write(f"  AUC: {test_cm.get('auc', 0):.2%}\n")
                else:
                    f.write("\nTEST DATASET CONFUSION MATRIX:\n")
                    f.write("-" * 35 + "\n")
                    f.write(f"  Data: {test_cm}\n")
        ...
        if metrics.get('task') == 'inflammation':
            ...
            if 'tile' in metrics.get('optimal_thresholds', {}):
                ...
                if isinstance(tile_metrics, dict):
                    ...
                    f.write(f"Tile-Level Metrics: {tile_metrics}\n")
            ...
            if 'slide' in metrics.get('optimal_thresholds', {}):
                ...
                if isinstance(slide_metrics, dict):
                    ...
                    f.write(f"Slide-Level Metrics: {slide_metrics}\n")
            ...
            if 'particle' in metrics.get('optimal_thresholds', {}):
                ...
                if isinstance(particle_metrics, dict):
                    ...
                    f.write(f"Particle-Level Metrics: {particle_metrics}\n")
        ...
        if metrics.get('task') == 'inflammation':
            ...
            # Inflammation Task - Slide level
            # FIXED: Use test confusion matrix metrics if available - guarantees consistency
            if slide_metrics:
                f.write(f"Slide-Level Accuracy: {slide_metrics.get('accuracy', 0):.2%}\n")
                f.write(f"Slide-Level Sensitivity: {slide_metrics.get('sensitivity', 0):.2%}\n")
                f.write(f"Slide-Level Specificity: {slide_metrics.get('specificity', 0):.2%}\n")
                f.write(f"Slide-Level F1 Score: {slide_metrics.get('f1', 0):.2%}\n")
                f.write(f"Slide-Level AUC: {slide_metrics.get('auroc', 0):.2%}\n")
            else:
            ...
            if particle_metrics:
                ...
                f.write(f"Particle-Level Specificity: {particle_metrics.get('specificity', 0):.2%}\n")
                f.write(f"Particle-Level F1 Score: {particle_metrics.get('f1', 0):.2%}\n")
                f.write(f"Particle-Level AUC: {particle_metrics.get('auroc', 0):.2%}\n")
            else:
                # Fall back to metrics dict
